...
function buildSvg(timeline: Timeline, scene: Scene, progress: number, subtitle: string): string {
  ...
  const mainText = renderSceneSvg(scene, style);
...
function renderSceneSvg(scene: Scene, style: Timeline["style"]): string {
  const items = cardItems(scene);
  if (scene.kind === "title") {
    return `${textLines(wrapText(scene.headline, 13), 150, 390, 96, style.ink, 900, "Georgia, serif")}
      ${textLines(wrapText(scene.accent ?? "", 24), 155, 570, 46, "#343044", 800)}
      ${pill(155, 685, "Buy Side")} ${pill(320, 685, "Sell Side")} ${pill(500, 685, "Quant")}`;
  }

  if (scene.kind === "tool") {
    return `${featureCard(150, 220, 760, 250, scene, style)}
      ${items.slice(0, 4).map((item, index) => infoCard(950 + (index % 2) * 390, 220 + Math.floor(index / 2) * 280, 360, 250, item, index)).join("\n")}`;
  }

  if (scene.kind === "quote" || scene.kind === "summary") {
    return `${featureCard(150, 220, 1460, 310, scene, style)}
      ${items.slice(0, 3).map((item, index) => infoCard(150 + index * 500, 570, 460, 210, item, index)).join("\n")}`;
  }

  return `${featureCard(150, 250, 720, 470, scene, style)}
    ${items.slice(0, 4).map((item, index) => infoCard(930 + (index % 2) * 360, 250 + Math.floor(index / 2) * 245, 330, 220, item, index)).join("\n")}`;
}
